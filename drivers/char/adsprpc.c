				...
				struct smq_invoke_ctx **po)
{
	...
	pr_info("adsprpc: context_alloc: sc=0x%x, bufs=%d, size=%d\n", (unsigned int)invoke->sc, bufs, size);
	...
	if (invokefd->fds) {
		if (!kernel) {
			...
			for (i = 0; i < bufs; i++)
				if (invokefd->fds[i])
					pr_info("adsprpc: context_alloc: i=%d, fd=%d, fd=%d\n", i, invokefd->fds[i], ctx->fds[i]);
		} else {
			...
			int *nbufs, int *fds, struct ion_handle **handles)
{
	...
	for (i = 0; i < inbufs + outbufs; ++i) {
		...
		if (me->smmu.enabled && fds && (fds[i] >= 0)) {
			...
			if (err)
				goto bail;
			pr_info("adsprpc: get_args: i=%d, fd=%d, handle=%d, iova=%x, len=%x\n", i, fds[i], (int)handles[i], (unsigned int)iova, (unsigned int)len);
