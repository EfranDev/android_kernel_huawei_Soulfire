...
#define GPIO_I2C_SDA_TP   2
#define GPIO_I2C_SCL_TP   3
...
static int cyttsp4_si_get_cydata(struct cyttsp4_core_data *cd)
{
	...
	if (mfgid_sz != calc_mfgid_sz) {
		tp_log_err( "%s: mismatch in MFGID size, reported:%d calculated:%d\n",
...
static irqreturn_t cyttsp4_irq(int irq, void *handle)
{
	...
	tp_log_vdebug( "%s mode[0-2]:0x%X 0x%X 0x%X\n", __func__,
...
static int cyttsp4_reset_checkout(struct cyttsp4_core_data *cd)
{
	...
	for (i = 0; i < CY_CORE_READTIMES; i++) {
		rc = cyttsp4_adap_read(cd, CY_REG_BASE,
			&buf, sizeof(buf));
		tp_log_debug( "%s: rc =%d\n", __func__,rc);
		if (rc < 0){
			tp_log_err("%s:I2C cannot communicate, no cypress device, exit!!!\n",__func__);
		}
		else {
		...
		u8 *cmd_buf, size_t cmd_size)
{
	...
	if (cmd_size > 1) {
		rc = cyttsp4_adap_write(cd, cmd_param_ofs, cmd_param_buf,
				cmd_param_size);
		if (rc < 0) {
			tp_log_err( "%s: Error on write command parameters r=%d\n",
				__func__, rc);
			return rc;
		}
	}
...
static int cyttsp4_get_ttconfig_info(struct cyttsp4_core_data *cd)
{
	...
	tp_log_debug( "%s: TT Config Version:%04X Length:%d Max Length:%d CRC:%04X\n",
...
extern int qpnp_force_enable_ldo14_by_spmi(bool on_off);
...
reset:
	...
	if (rc < 0) {
		...
		tp_log_err( "%s: begin to reset the power of vbus\n", __func__);
		gpio_set_value(cd->pdata->rst_gpio, 0);

        /* use force interface instead of standard interface */
        rc= qpnp_force_enable_ldo14_by_spmi(false);
        //rc = regulator_disable(vbus_cypress);
		if(rc < 0){
		    tp_log_err( "%s: failed to disable cypress vbus by force\n", __func__);
		    return  -EINVAL;
		}
		mdelay(2000);

		//rc = regulator_enable(vbus_cypress);
        rc= qpnp_force_enable_ldo14_by_spmi(true);
		if (rc < 0) {
		   tp_log_err( "%s: failed to enable cypress vdd by force\n", __func__);
		   return -EINVAL;
		}
		mdelay(100);
...
static int add_sensitivity_sysfs_interfaces(struct device *dev)
{
	...
	if(error)
	{
		tp_log_err( "%s: failed to create glove sysfs_link ,error = %d\n", __func__,error);
		goto undo;
	}
	return 0;
undo:
...
static int cyttsp4_core_probe(struct cyttsp4_core *core)
{
	...
	gp_rc = gpio_request(GPIO_I2C_SDA_TP, "cyttsp_gpio");
    if (gp_rc < 0) {
    	gpio_free(GPIO_I2C_SDA_TP);
    	gp_rc = gpio_request(GPIO_I2C_SDA_TP,"cyttsp_gpio");
	}

    if( gp_rc < 0 )
    {
    	tp_log_err("***********************data: fail to request gpio 2!\n");
    }

    gp_rc = gpio_tlmm_config(GPIO_CFG(GPIO_I2C_SDA_TP,0,GPIO_CFG_INPUT,GPIO_CFG_PULL_DOWN,GPIO_CFG_8MA),GPIO_CFG_ENABLE);
    if(gp_rc < 0) {
    	tp_log_err("***********************data: fail to set gpio 2 pull down!\n");
    }

    gvalue = gpio_get_value(GPIO_I2C_SDA_TP);
    printk("***********************data: gpio 2 value is %d\n", gvalue);
        
    gp_rc = gpio_request(GPIO_I2C_SCL_TP, "cyttsp_gpio");
    if (gp_rc < 0) {
    	gpio_free(GPIO_I2C_SCL_TP);
    	gp_rc = gpio_request(GPIO_I2C_SCL_TP,"cyttsp_gpio");
	}

    if( gp_rc < 0 )
    {
   		tp_log_err("***********************clk: fail to request gpio 3!\n");
    }

    gp_rc = gpio_tlmm_config(GPIO_CFG(GPIO_I2C_SCL_TP,0,GPIO_CFG_INPUT,GPIO_CFG_PULL_DOWN,GPIO_CFG_8MA),GPIO_CFG_ENABLE);
    if(gp_rc < 0) {
    	tp_log_err("***********************clk: fail to set gpio 3 pull down!\n");
    }

    gvalue = gpio_get_value(GPIO_I2C_SCL_TP);
    tp_log_err("***********************clk: gpio 3 value is %d\n", gvalue);    
