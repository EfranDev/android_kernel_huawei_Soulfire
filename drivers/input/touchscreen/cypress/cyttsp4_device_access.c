		...
		size_t length, int num_read, int offset, int mode)
{
	...
	if (length < num_read) {
		tp_log_err( "%s: not sufficient buffer req_bug_len=%d, length=%d\n",
		...
		size_t length)
{
	...
	if (length < return_buf_size) {
		tp_log_err( "%s: not sufficient buffer req_buf_len=%d, length=%d\n",
		...
		u8 *ic_buf, size_t length)
{
	...
	if (pdata->sett[CY_IC_GRPNUM_TCH_PARM_SIZE] == NULL) {
		tp_log_err( "%s: Missing platform data Touch Parameters Sizes table\n",
				__func__);
		return -EINVAL;
	}

	if (pdata->sett[CY_IC_GRPNUM_TCH_PARM_SIZE]->data == NULL) {
		tp_log_err( "%s: Missing platform data Touch Parameters Sizes table data\n",
				__func__);
		return -EINVAL;
	}
		...
		size_t length)
{
	...
	if (dad->test.cur_cmd == CY_CMD_CAT_NULL) {
		...
		tp_log_debug( "%s: GRP=TEST_REGS: num_rd=%d at ofs=%d + grpofs=%d\n",
		...
		if (length > 1) {
			rc = cyttsp4_write(dad->ttsp,
				...
				dad->ic_grpoffset + dad->si->si_ofs.cmd_ofs
					+ 1, ic_buf + 1, length - 1);
			if (rc < 0) {
				tp_log_err( "%s: Fail write cmd param regs r=%d\n",
					__func__, rc);
				return 0;
			}
		}
		...
		size_t buf_size, u8 *ic_buf, size_t ic_buf_size)
{
	...
	tp_log_debug( "%s: pbuf=%p buf=%p size=%d %s=%d buf=%s\n", __func__,
		...
		struct device_attribute *attr, char *buf)
{
	...
	if (rc < 0) {
		tp_log_err( "%s: Error on _cyttsp4_ret_scan_data_cmd(), offset=%d num_element:%d\n",
			__func__, read_element_offset, left_over_element);
		goto cyttsp4_get_panel_data_show_err_release;
	}
	if (return_buf[CY_CMD_OUT_STATUS_OFFSET] != CY_CMD_STATUS_SUCCESS) {
		tp_log_err( "%s: Fail on _cyttsp4_ret_scan_data_cmd(), offset=%d num_element:%d\n",
			__func__, read_element_offset, left_over_element);
		goto cyttsp4_get_panel_data_show_err_release;
	}
	...
	tp_log_info( "%s: left_over_element=%d, read_element_offset=%d,data_idx=%d\n", __func__, left_over_element, read_element_offset,data_idx);
	...
	while (left_over_element > 0) {
		...
		rc = _cyttsp4_ret_scan_data_cmd(dev, read_element_offset,
				...
				return_buf);
		if (rc < 0) {
			tp_log_err( "%s: Error %d  on _cyttsp4_ret_scan_data_cmd(), offset=%d num_element:%d\n",
		...
		if (returned_element == 0) {
			tp_log_debug( "%s: returned_element=0, left_over_element=%d\n",
	...
	switch(type)
	{
		case CY_CHK_MUT_RAW:
			...
			for (i=0; i < capacitance_data.ignore_list_size_mut_cap; ++i)
			{
				...
				{
					printk("%s Index %d is ignored in Mutual capacitance \n", __func__,index);
					return 1;
				}
			}
			break;
		...
		case CY_CHK_SELF_RAW:
			for (i=0; i < capacitance_data.ignore_list_size_self_cap; ++i)
			{
				...
				{
					printk("%s Index %d is ignored in Self capacitance \n", __func__,index);
					return 1;
				}
			}
			break;
...
static int out_of_range(enum check_data_type type, int value, int index)
{
	...
	{
    	case CY_CHK_MUT_RAW:    
    		if(value < capacitance_data.min_mut_cap || value > capacitance_data.max_mut_cap)
    		{
    			return 1;
    		}
    		break;
        ...
        case CY_CHK_SELF_RAW:
    		if(value < capacitance_data.min_self_cap || value > capacitance_data.max_self_cap )
    		{
    			return 1;
    		}
    		break;
		...
		if (endian == B_ENDIAN)
		{
			if (0 == index%datasize) 	//high byte
			{
				...
				if (datasize ==1)
				{
					...
					if (out_of_range(type, temp, (index - capacitance_data.data_start_byte)/datasize))
					{
...
static int cyttsp4_get_data_and_check(struct device* dev, retrieve_func ret_func, enum check_data_type type, int offset)
{
	...
	if(read_byte > CY_MAX_READ_DATA)
	{
		...
		left_over_element = dad->heatmap.num_element - CY_MAX_SENSOR_READ_DATA;
...
static int cyttsp4_check_raw_data(struct device *dev)
{
   	...
   	for(i = 0; i < 2; ++i)
    {
		...
		{	
			type = CY_CHK_MUT_RAW;
			dad->heatmap.num_element = (capacitance_data.tx_lines*capacitance_data.rx_lines); 
			dad->heatmap.data_type = CY_MUT_RAW;
		}
        else
		{	
			type = CY_CHK_SELF_RAW;
			dad->heatmap.num_element = capacitance_data.rx_lines;
...
int  cyttsp4_get_panel_data_check(char **buf)
{
	...
	if(rc <= 0){
		tp_log_err( "%s: Error on cyttsp4_check_raw_data r=%d\n",
				__func__, rc);
		goto cyttsp4_get_panel_data_show_err_release;
	}
...
/* disable short test function */
	...
	tp_log_err("%s:begin to cyttsp4_check_short_data check\n",__func__);
...
{
	int rc = 0;
	/*do once calibration and then attempt to test again  if tset fail first time*/
	...
	/*If its failed retry after calibration*/
	if (0 == strcmp(touch_mmi_test_result,"FAIL")){
		tp_log_err("%s: cyttsp4_get_panel_data_check Fail,calibrate and attempt to test again\n", __func__);
		...
		if (NULL != dad)
		{
			...
			rc = cyttsp4_get_panel_data_check(&touch_mmi_test_result);
			if(rc < 0){
				tp_log_err("cyttsp4_get_panel_data_check error 2\n");
			}
		}
	}
...
static int cyttsp4_device_access_release(struct cyttsp4_device *ttsp)
{
	...
	if (dad->own_exclusive) {
		tp_log_err( "%s: Can't unload in CAT mode. First switch back to Operational mode\n"
				, __func__);
...
static int __init cyttsp4_device_access_init(void)
{
	...
	for (i = 0; i < num_core_ids; i++) {
		...
		cyttsp4_device_access_infos[i].platform_data =
			&_cyttsp4_device_access_platform_data;
		tp_log_debug("%s: Registering device access device for core_id: %s\n",
