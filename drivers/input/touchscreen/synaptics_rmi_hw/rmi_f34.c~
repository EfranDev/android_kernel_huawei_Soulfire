...
static int i2c_update_firmware(struct rmi_function_container *fc, const char *filename);
...
static int synaptics_get_cob_tp_id(struct rmi_function_container *fc);
static int synaptics_get_cof_tp_id(struct rmi_function_container *fc);
...
static int upgrade_firmware(struct rmi_function_container *fc, const char *filename)
{
	int irq = 0;
	int retval = 0;
...
static int rmi_update_firmware(struct rmi_function_container *fc)
{
    ...
    printk("enter rmi_update_firmware,want to update ver is %d,local ver is %d\n",pdata->pr_version,f34->firmwareid);
    if(NEED_UPDATE_FW == is_need_update_fw())
    {
    	if(pdata->pr_version != f34->firmwareid)
    	{
    		retval = upgrade_firmware(fc,pdata->firmware_name);
			if( 0 != retval )
			{
				printk("Update firmware failed!\n");
				retval = -1;
...
static int auto_update_config(struct rmi_function_container *fc)
{
	...
	if(TP_COB == pdata->ic_position)
	{
		module_id = synaptics_get_cob_tp_id(fc);
	}
	else if (TP_COF == pdata->ic_position)
	{
		module_id = synaptics_get_cof_tp_id(fc);
	}
	else
		
	printk("module_id is %d \n",module_id);
	if (module_id < 0)
	{
	...
	pconfig_array = get_tp_version_config(module_id,f34->ic_name);
	...
	if ((NULL !=pconfig_array) && (pdata->pr_version == f34->firmwareid))
	{
...
static int synaptics_get_cob_tp_id(struct rmi_function_container *fc)
{
	...
	sprintf(touch_info,"synaptics_rmi_%s.%d",get_touch_module_name(TP_COB,synaptics_module_id),configid[3]);
...
static int synaptics_get_cof_tp_id(struct rmi_function_container *fc)
{
	unsigned long module_id = TP_COF_ID_UNKNOW;
	char productid[1] = {0};
	int retval = 0; 
	unsigned char configid[5] = {0};
	u16 control_base_addr = fc->fd.control_base_addr;
	...
	/*get firmware version*/
	retval = rmi_read_block(fc->rmi_dev,f01_fc->fd.query_base_addr + 20,&productid[0], 1);
	if (retval < 0) 
	{
		dev_err(&fc->dev, "Failed to read package ID.\n");
		return retval;
	...
	retval = strict_strtoul(&productid[0], 10, &module_id);
	if (retval)
	{
	...
	synaptics_module_id = module_id;
	sprintf(touch_info,"synaptics_rmi_%s.%d",get_touch_module_name(TP_COF,(u8)module_id),configid[3]);
	retval = app_info_set("touch_panel", touch_info);
	if (retval) 
	{
		printk( "set_touch_chip_info error\n");
	}
