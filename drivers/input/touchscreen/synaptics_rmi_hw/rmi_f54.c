 ...
 */
/* close watch dog */
#define F54_WATCHDOG 0

DECLARE_COMPLETION(mmi_comp); 
DECLARE_COMPLETION(mmi_sync);
...
#define MAX_F54_RAW_DATA_BUF_LEN 3000
...
static char g_RxtoRxshort_report[24 * 24 * 7] = {0};
static u8 g_tx_max = 0;
static char mmi_buf[2000] = {0};
...
static void mmi_rxtorxshort_test(struct device * dev,size_t count)
{
	...
	/* add delay and remove no nedd code */
...
/*move to init*/
    ...
    printk("f54_rawimage_report moudle id is %d\n",moudle_id);
...
static int f54_RxtoRxshort_report(struct rmi_fn_54_data *data,size_t count)
{
	...
	int DiagonalUpperLimit = 1348;
	int DiagonalLowerLimit = 688;
...
{
	/* change threshold get from vn1 */
	short ReceiverMax = 315;
	short TransmiterMax = 162;
...
static int rmi_f54_initialize(struct rmi_function_container *fc)
{
	...
	/* remove Multithreading */
	__mutex_init(&instance_data->test_mutex, "test_mutex",
...
int rmi_f54_attention(struct rmi_function_container *fc, u8 *irq_bits)
{
	...
	if ( IC_TYPE_3207 == ic_name)
		error = rmi_read_block(fc->rmi_dev,f01_fc->fd.control_base_addr+ RX_NUMBER+1,
			&rx, 1);
	else//2202 use this
	...
	if ( IC_TYPE_3207 == ic_name)
		error = rmi_read_block(fc->rmi_dev,f01_fc->fd.control_base_addr+ TX_NUMBER+1,
			&tx, 1);
	else//2202 use this
