...
***************************************************************************/
static uint16_t s5k4e1ga_read_current(struct msm_sensor_ctrl_t *s_ctrl,uint16_t otpAddr, uint16_t MSBorLSB)
{
    uint16_t otpData = 0;
    ...
    CDBG("%s: startCurrentt=%d , macroCurrent=%d \n", __func__, startCurrent, macroCurrent);
    
    if((0 != startCurrent) && (0 != macroCurrent))
    {
...
/* fcution:  read VCM start and end current
   return 0: startCurrent or cacroCurrent maybe failed
   return 1: startCurrent and cacroCurrent both ok
*/
static uint16_t s5k4e1ga_read_vcm_current(struct msm_sensor_ctrl_t *s_ctrl)
{
    int rc = 0;
    
    /*read 0x310D LSB*/
    rc = s5k4e1ga_read_current(s_ctrl, 0x310D, S5K4E1GA_OTP_LSB);
    if(0 != rc)
    {
        return rc;
    }
    ...
    /*read 0x310E MSB*/
    rc = s5k4e1ga_read_current(s_ctrl, 0x310E, S5K4E1GA_OTP_MSB);
    if(0 != rc)
    {
        return rc;
    }
    ...
    /*read 0x310E LSB*/
    rc = s5k4e1ga_read_current(s_ctrl, 0x310E, S5K4E1GA_OTP_LSB);
    if(0 != rc)
    {
        return rc;
    }
    ...
    /*read 0x310F MSB*/
    rc = s5k4e1ga_read_current(s_ctrl, 0x310F, S5K4E1GA_OTP_MSB);
    if(0 != rc)
    {
        return rc;
    }
    ...
    /*read 0x310F LSB*/
    rc = s5k4e1ga_read_current(s_ctrl, 0x310F, S5K4E1GA_OTP_LSB);
    if(0 != rc)
    {
        return rc;
    }
...
*/
static int s5k4e1ga_otp_check_awb(struct msm_sensor_ctrl_t *s_ctrl, uint16_t *pAddr, uint16_t *pMSBorLSB)
{
    uint16_t otpData = 0;
    uint16_t otpAddr = 0;
...
static int s5k4e1ga_otp_data_read(struct msm_sensor_ctrl_t *s_ctrl)
{
    ...
    /*check awb */
    if(0 == s5k4e1ga_otp_check_awb(s_ctrl, &otpAddr, &MSBorLSB))
    {
    	return -1;
    }
    
    rg = s5k4e1ga_read_rg(s_ctrl, otpAddr, MSBorLSB);
    bg = s5k4e1ga_read_bg(s_ctrl, otpAddr, MSBorLSB);
    
    /* no vcm info */
    if(0 == s5k4e1ga_read_vcm_current(s_ctrl))
    {
    	return -1;
    }
