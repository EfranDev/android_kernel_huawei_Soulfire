...
static int msm8x10_wcd_volatile(struct snd_soc_codec *codec, unsigned int reg)
{
	...
	/* Values of bandgap registers may change */
	if (reg == MSM8X10_WCD_A_BIAS_REF_CTL ||
	    reg == MSM8X10_WCD_A_BIAS_CENTRAL_BG_CTL)
		return 1;
...
static
#endif
unsigned int msm8x10_wcd_read(struct snd_soc_codec *codec,
				unsigned int reg)
{
	unsigned int val;
	int ret;
...
#ifdef CONFIG_SOUND_CONTROL_HAX_3_GPL
EXPORT_SYMBOL(msm8x10_wcd_read);
#endif
